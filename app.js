...
async function fetchQuestions() {
  try {
    ...
    questions = data.results.map(q => {
      const choices = [...q.incorrect_answers, q.correct_answer].map(c => decodeURIComponent(c));
...
async function showQuestion() {
  ...
  if (language === 'es') {
    if (!q.question_es) q.question_es = await translateText(q.question_en, 'en', 'es');
    for (let i = 0; i < q.choices_en.length; i++) {
      if (!q.choices_es[i]) {
        q.choices_es[i] = await translateText(q.choices_en[i], 'en', 'es');
      }
    }
  }
